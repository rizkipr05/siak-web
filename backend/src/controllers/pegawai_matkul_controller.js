...
const getMatkulList = async (req, res) => {
    try {
        const [rows] = await db.query(`
            SELECT KodeMK, NamaMatkul, SKS, Prasyarat, Semester
            FROM matakuliah
...
const createMatkul = async (req, res) => {
    const { KodeMK, NamaMatkul, SKS, Prasyarat, Semester } = req.body;
    if (!KodeMK || !NamaMatkul || !SKS || !Semester) {
        return res.status(400).json({ success: false, message: 'Data tidak lengkap' });
    }
    try {
        await db.query(
            `INSERT INTO matakuliah (KodeMK, NamaMatkul, SKS, Prasyarat, Semester) VALUES (?, ?, ?, ?, ?)`,
            [KodeMK, NamaMatkul, SKS, Prasyarat || null, Semester]
        );
...
const updateMatkul = async (req, res) => {
    ...
    const { NamaMatkul, SKS, Prasyarat, Semester } = req.body;
    try {
        await db.query(
            `UPDATE matakuliah SET NamaMatkul = ?, SKS = ?, Prasyarat = ?, Semester = ? WHERE KodeMK = ?`,
            [NamaMatkul, SKS, Prasyarat || null, Semester, kode]
        );
