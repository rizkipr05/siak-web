...
import { Sparkles, CalendarCheck, UserCheck } from 'lucide-react';
...
const AbsensiSaya = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${apiBase}/api/mahasiswa/absensi`, {
    ...
    <div className="w-full space-y-8 animate-in fade-in duration-700 pb-20">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="space-y-4 text-center md:text-left">
            ...
            <h1 className="text-4xl md:text-6xl font-black tracking-tighter uppercase leading-none">
              Absensi <span className="text-emerald-500 italic">Saya</span>
            </h1>
          </div>
          <div className="bg-white/5 backdrop-blur-md px-8 py-5 rounded-[2.5rem] border border-white/10 flex items-center gap-5">
            ...
            <div className="text-left">
              ...
              <p className="text-2xl font-bold text-white leading-tight">
                {loading ? '-' : data.length} <span className="text-xs text-emerald-500 uppercase">Sesi</span>
              </p>
            </div>
          </div>
        </div>
      ...
      {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  ...
                  <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Pertemuan</th>
                  <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Tanggal</th>
                ...
                {data.map((a, idx) => (
                  <tr key={idx} className="hover:bg-emerald-50/30 transition-colors">
                    <td className="px-8 py-6">
                      <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">{a.KodeMK}</p>
                      <h4 className="text-sm font-black text-slate-800 uppercase tracking-tight">{a.NamaMatkul}</h4>
                    </td>
                    <td className="px-6 py-6 text-center font-black">#{a.pertemuan_ke}</td>
                    <td className="px-6 py-6 text-center">{a.tanggal?.slice(0, 10)}</td>
                    <td className="px-6 py-6 text-center">
                      <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-widest">
                        <UserCheck size={12} /> {a.status_hadir}
                      </span>
                    </td>
                  </tr>
                ))}
        ...
        <div className="bg-white py-32 rounded-[3rem] border-2 border-dashed border-slate-100 text-center flex flex-col items-center">
          <CalendarCheck size={48} className="text-slate-100 mb-6" />
          <h3 className="text-xl font-black uppercase text-slate-300 tracking-[0.2em]">Data Tidak Ditemukan</h3>
          <p className="text-slate-400 text-[10px] font-bold mt-2 uppercase tracking-widest">Belum ada absensi</p>
        </div>
      )}
    </div>
  );
