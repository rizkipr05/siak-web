...
import { 
  ...
  FileText, Loader2, User, Calendar, Award
} from 'lucide-react';
...
const CetakKHS = () => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `KHS_${npm}_${selectedPeriod?.replace(/\//g, '-')}`,
  });

  const handleSelectAndPrint = (periodKey: string) => {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/admin/reports/khs?npm=${npm}`, {
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="space-y-4 text-center md:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-[0.3em]">
              <Sparkles size={12} className="animate-pulse" /> ACADEMIC TRANSCRIPT
            </div>
            <h1 className="text-4xl md:text-6xl font-black tracking-tighter uppercase leading-none">
              CETAK <span className="text-indigo-500 italic">KHS</span>
            </h1>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 bg-slate-50 p-3 rounded-[2.5rem] border border-slate-100 shadow-inner">
            <div className="relative flex-1">
              ...
              <input 
                ...
                placeholder="MASUKKAN NPM MAHASISWA (CONTOH: 211510...)" 
          ...
          <div className="relative min-h-[400px]">
            {loading ? (
                        ...
                        {Object.keys(groupedData).map((periodKey) => (
                          <tr key={periodKey} className="group hover:bg-slate-50 transition-all">
                            ...
                            <td className="px-10 py-8 text-center">
                              <span className="px-4 py-2 bg-indigo-50 rounded-xl text-[10px] font-black text-indigo-600">
                                TERSEDIA
                              </span>
                            </td>
                            <td className="px-10 py-8 text-right">
                              <button 
                                onClick={() => handleSelectAndPrint(periodKey)}
              ...
              <div className="bg-white py-40 rounded-[4rem] border-4 border-dashed border-slate-50 text-center flex flex-col items-center">
                <GraduationCap size={48} className="text-rose-200 mb-6" />
                <h3 className="text-2xl font-black uppercase text-slate-300 tracking-[0.3em]">Tidak Ditemukan</h3>
                <p className="text-slate-400 text-[11px] font-bold mt-2 uppercase tracking-widest max-w-[300px] leading-relaxed">
                  Mahasiswa dengan NPM <span className="text-indigo-600 font-black">{npm}</span> belum memiliki nilai divalidasi.
                </p>
              </div>
            ) : (
              <div className="bg-white py-40 rounded-[4rem] border-4 border-dashed border-slate-50 text-center flex flex-col items-center">
                ...
                <p className="text-slate-400 text-[11px] font-bold mt-2 uppercase tracking-widest max-w-[280px]">Masukkan NPM untuk memproses pencetakan dokumen KHS.</p>
              </div>
            )}
          </div>
        </>
