...
const InputDosen = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/staff/dosen-list');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEdit 
        ? `http://localhost:5000/api/staff/update-dosen/${formData.NIDN}`
        : 'http://localhost:5000/api/staff/input-dosen';
  ...
  const handleEdit = (d: any) => {
    ...
    setFormData({
      ...
      Nama: d.Nama,
      Gelar: d.Gelar || '',
      // PASTIKAN d.JenisKelamin (huruf besar/kecil sesuai database)
  ...
  const executeDelete = async (nidn: string) => {
    ...
    setActionLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/staff/delete-dosen/${nidn}`, { method: 'DELETE' });
      const result = await res.json();
      if (result.success) {
                        ...
                        {filteredDosen.map((d) => (
                            ...
                            <td className="px-10 py-6 flex items-center gap-4">
                                ...
                                <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">
                                    {d.NIDN} • <span className={d.JenisKelamin === 'L' ? 'text-blue-500' : 'text-rose-500'}>{d.JenisKelamin === 'L' ? 'Laki-laki' : 'Perempuan'}</span>
                                </p>
                                </div>
                            </td>
