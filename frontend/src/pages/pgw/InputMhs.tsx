...
const InputMhs = () => {
  ...
  const fetchData = async () => {
    try {
      const resMhs = await fetch('http://localhost:5000/api/staff/mahasiswa-list');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEdit 
        ? `http://localhost:5000/api/staff/update-mahasiswa/${formData.NPM}`
        : 'http://localhost:5000/api/staff/input-mahasiswa';
  ...
  const executeDelete = async (npm: string) => {
    ...
    setActionLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/staff/delete-mahasiswa/${npm}`, { method: 'DELETE' });
      const result = await res.json();
      if (result.success) {
