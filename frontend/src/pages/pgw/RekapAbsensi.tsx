...
const RekapAbsensi = () => {
  ...
  useEffect(() => {
    const fetchJadwal = async () => {
      try {
        ...
        const res = await fetch('http://localhost:5000/api/staff/jadwal-aktif'); 
  ...
  const fetchAbsensi = async (jadwal: any, pertemuan: string = "all") => {
    ...
    try {
      const url = `http://localhost:5000/api/staff/rekap-absensi?id_jadwal=${jadwal.id_jadwal}&pertemuan=${pertemuan}`;
